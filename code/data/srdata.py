...
class SRData(data.Dataset):
    def __init__(self, args, train=True, benchmark=False):
        ...
        if args.ext == 'img' or benchmark:
            ...
            try:
                ...
                list_hr, list_lr, list_lrb = self._scan()
    ...
    def _get_patch(self, lr, hr, lrb):
        patch_size = self.args.patch_size
        scale = self.scale[self.idx_scale]
        multi_scale = len(self.scale) > 1
        if self.train:
            lr, hr, lrb = common.get_patch(
                lr, hr, lrb, patch_size, scale, multi_scale=multi_scale
            )
            lr, hr, lrb = common.augment([lr, hr, lrb])
